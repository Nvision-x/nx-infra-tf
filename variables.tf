...
variable "enable_cloudtrail_s3_data_events" {
  description = "Create the nvisionx-s3-data-events trail. Defaults to false: S3 data events bill per object operation, so in a shared or customer-owned account this trail can cost thousands per month for controls that may not be required. Requires enable_security_hub_controls for its bucket and KMS key."
  type        = bool
  default     = false
}
