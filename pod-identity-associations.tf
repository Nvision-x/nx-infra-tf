...
resource "aws_eks_pod_identity_association" "cluster_autoscaler" {
  count = var.cluster_autoscaler_role_arn != "" ? 1 : 0
...
resource "aws_eks_pod_identity_association" "postgres_backup" {
  count = var.enable_postgres && var.postgres_backup_role_arn != "" ? 1 : 0
...
locals {
  ...
  app_s3_sa_pairs = var.enable_app_s3_access && var.app_s3_role_arn != "" ? [
...
resource "aws_eks_pod_identity_association" "app_s3" {
  for_each = { for idx, sa in local.app_s3_sa_pairs : "${sa.namespace}-${sa.service_account}" => sa }
...
locals {
  ...
  bedrock_sa_pairs = var.enable_bedrock_irsa && var.bedrock_role_arn != "" ? [
...
resource "aws_eks_pod_identity_association" "bedrock" {
  for_each = { for idx, sa in local.bedrock_sa_pairs : "${sa.namespace}-${sa.service_account}" => sa }
